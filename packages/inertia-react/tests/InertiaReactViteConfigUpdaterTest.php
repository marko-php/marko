...
beforeEach(function (): void {
    $this->tempDirectory = sys_get_temp_dir() . '/marko-inertia-react-vite-config-' . bin2hex(random_bytes(6));
...
function makeInertiaReactViteConfigUpdater(string $directory): InertiaReactViteConfigUpdater
{
    ...
    return new InertiaReactViteConfigUpdater(
        $viteConfig,
...
test('inertia react vite config updater creates a react-aware root vite config when missing', function (): void {
...
test('inertia react vite config updater upgrades the default vite stub', function (): void {
    file_put_contents(
...
test('inertia react vite config updater preserves custom tailwind entrypoints', function (): void {
    ...
    $result = makeInertiaReactViteConfigUpdater($this->tempDirectory)->ensureReactConfig();
