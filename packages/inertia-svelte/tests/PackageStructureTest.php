...
it('has a valid composer.json for marko/inertia-svelte', function (): void {
    $composer = json_decode(file_get_contents(dirname(__DIR__) . '/composer.json'), true, flags: JSON_THROW_ON_ERROR);

    expect($composer['name'])->toBe('marko/inertia-svelte')
        ...
        ->and($composer['autoload']['psr-4']['Marko\\Inertia\\Svelte\\'])->toBe('src/')
        ->and($composer['autoload-dev']['psr-4']['Marko\\Inertia\\Svelte\\Tests\\'])->toBe('tests/');
});

it('has module.php with bindings for the inertia svelte services', function (): void {
    $module = require dirname(__DIR__) . '/module.php';

    expect($module)->toBeArray()
        ->and($module)->toHaveKey('bindings')
        ->and($module['bindings'])->toHaveKey(InertiaSveltePublisherInterface::class)
        ->and($module['bindings'][InertiaSveltePublisherInterface::class])->toBe(InertiaSveltePublisher::class)
        ->and($module['singletons'])->toContain(InertiaSveltePublisher::class)
        ->and($module['singletons'])->toContain(InertiaSvelteViteConfigUpdater::class);
});
