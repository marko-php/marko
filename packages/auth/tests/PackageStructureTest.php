...
it('creates marko/auth package with valid composer.json', function () {
...
it('has correct package name marko/auth', function () {
    $composerPath = dirname(__DIR__) . '/composer.json';
    $composer = json_decode(file_get_contents($composerPath), true);

    expect($composer['name'])->toBe('marko/auth');
});
...
it('has PSR-4 autoloading configured for Marko\Auth namespace', function () {
    $composerPath = dirname(__DIR__) . '/composer.json';
    $composer = json_decode(file_get_contents($composerPath), true);

    expect($composer)->toHaveKey('autoload')
        ->and($composer['autoload'])->toHaveKey('psr-4')
        ->and($composer['autoload']['psr-4'])->toHaveKey('Marko\\Auth\\')
        ->and($composer['autoload']['psr-4']['Marko\\Auth\\'])->toBe('src/');
});
