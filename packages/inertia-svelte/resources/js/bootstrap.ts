...
export function bootstrapMarkoInertiaSvelte(
  ...
  const config: MarkoInertiaSvelteConfig = {
    ...
    layout: (component, page) => {
      ...
      return (
        resolveServerLayout(options, normalizedPage, component, parsed) ??
...
function normalizeSveltePage(
  resolved: unknown,
): MarkoInertiaSveltePage {
  if (isObjectLike(resolved) && "default" in resolved && isPageLike(resolved.default)) {
    return resolved.default;
  }

  if (isPageLike(resolved)) {
    return resolved;
  }

  throw new Error(
    "Resolved Inertia Svelte pages must export a component function/object or a module with a default export.",
  );
}

function isPageLike(value: unknown): value is MarkoInertiaSveltePage {
