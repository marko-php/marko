...
it(
    'both drivers throw InvalidColumnException for orderByRaw denylist inputs (semicolon, --, /*, */, backtick)',
    function (string $dangerous): void {
        $mysqlConnection = makeRecordingConnection();
        $pgsqlConnection = makeRecordingConnection();
...
it(
    'the emitted SQL contains the same ORDER BY clause across both drivers (raw expression with direction)',
    function (): void {
        $mysqlConnection = makeRecordingConnection();
        $pgsqlConnection = makeRecordingConnection();
