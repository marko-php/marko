...
it('has composer.json with name marko/devai and dependencies on mcp and lsp', function (): void {
    ...
    expect(file_exists($composerPath))->toBeTrue()
        ->and($composer['name'])->toBe('marko/devai')
        ->and($composer['require'])->toHaveKey('marko/mcp')
        ->and($composer['require'])->toHaveKey('marko/lsp')
...
it('autoloads cleanly with composer dump-autoload', function (): void {
    ...
    expect($composer['autoload']['psr-4'])->toHaveKey('Marko\\DevAi\\')
        ->and($composer['autoload']['psr-4']['Marko\\DevAi\\'])->toBe('src/')
        ->and($composer['autoload-dev']['psr-4'])->toHaveKey('Marko\\DevAi\\Tests\\')
