...
it(
    'it reflects a newly added app module on the next read after the data was already loaded',
    function () use (&$tmpDirs): void {
        $rootPath = makeTempDir();
        $tmpDirs[] = $rootPath;
...
it(
    'it reflects a newly added route via getRoutes after the data was already loaded',
    function () use (&$tmpDirs): void {
        $rootPath = makeTempDir();
        $tmpDirs[] = $rootPath;
...
it(
    'it reflects an edit to a tracked app or modules source file on the next read after first load',
    function () use (&$tmpDirs): void {
        $rootPath = makeTempDir();
        $tmpDirs[] = $rootPath;
...
it(
    'it reflects a deleted app module on the next read by comparing the tracked path set',
    function () use (&$tmpDirs): void {
        $rootPath = makeTempDir();
        $tmpDirs[] = $rootPath;
...
it(
    'it does not trigger a rebuild when only a vendor file changes after first load',
    function () use (&$tmpDirs): void {
        $rootPath = makeTempDir();
        $tmpDirs[] = $rootPath;
...
it(
    'it rebuilds at most once for several successive reads following a single change',
    function () use (&$tmpDirs): void {
        $rootPath = makeTempDir();
        $tmpDirs[] = $rootPath;
...
it(
    'it treats a cache payload missing the tracked path set as stale so old caches rebuild once',
    function () use (&$tmpDirs): void {
        $rootPath = makeTempDir();
        $tmpDirs[] = $rootPath;
...
it(
    'it persists the tracked path set as a plain array or string that requires no new unserialize allowed class',
    function () use (&$tmpDirs): void {
        $rootPath = makeTempDir();
        $tmpDirs[] = $rootPath;
...
it(
    'it preserves first-load semantics by loading a fresh on-disk cache without rescanning',
    function () use (&$tmpDirs): void {
        $rootPath = makeTempDir();
        $tmpDirs[] = $rootPath;
